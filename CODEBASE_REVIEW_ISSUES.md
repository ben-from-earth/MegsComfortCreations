...
### `database.save` not fully transactional (partial)

- **Paths**
  - `lib/trpc/routers/database/_.ts`
  - `lib/media-storage/media-image-records.ts`
- **Status**
  - Compensating deletes exist when image persistence fails after parent insert.
  - Production path still does not use `db.transaction`; S3 writes cannot roll back with DB.
- **Fix suggestion**
  - Wrap multi-step DB writes in a transaction.
  - Document/handle orphaned S3 objects on partial failure.

### Concurrent-delete edge in `database.edit` (partial)

- **Path**
  - `lib/trpc/routers/database/_.ts`
- **Issue**
  - Existence check then update; returned row may be missing if deleted concurrently, but code assumes an id.
- **Fix suggestion**
  - Treat empty update result as `NOT_FOUND` instead of asserting on the row.
...
- **Paths**
  ...
  - `__tests__/trpc/database.router.test.ts`, `__tests__/trpc/genres.router.test.ts` — brittle Drizzle fluent mocks
...
6. Wrap `database.save` DB steps in a transaction; harden `database.edit` empty-result handling.
7. Remove duplicate smoke tests; fix Windows `test:ci`; add test typecheck to CI.
